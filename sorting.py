def find_min_index(A, k):
    ...
    # YOUR CODE HERE
    spliced_list = [A[i] for i in range(k, len(A))]
    sorted_list = sorted(spliced_list)
    return spliced_list.index(sorted_list[0]) + k
...
def sort_by_key(country_list, key_function):
    ...
    # DON'T CHANGE ANYTHING ABOVE
    # YOUR CODE HERE
    sorted_final = sorted(country_list,key = get_unemployment, reverse = True)
    return sorted_final
